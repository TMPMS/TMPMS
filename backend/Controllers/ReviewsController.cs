    ...
    public class ReviewsController : ControllerBase
    {
        ...
        public async Task<IActionResult> CheckEligibility([FromQuery] int medicineId, [FromQuery] int userId)
        {
            var hasPurchased = await _context.Orders
                .Where(o => o.UserId == userId && (o.PaymentStatus == "Paid" || o.Status == "Completed"))
                .AnyAsync(o => _context.OrderItems.Any(oi => oi.OrderId == o.Id && oi.MedicineId == medicineId));
        ...
        public async Task<IActionResult> CreateReview([FromBody] CreateReviewInput input)
        {
            ...
            var hasPurchased = await _context.Orders
                .Where(o => o.UserId == input.UserId && (o.PaymentStatus == "Paid" || o.Status == "Completed"))
                .AnyAsync(o => _context.OrderItems.Any(oi => oi.OrderId == o.Id && oi.MedicineId == input.MedicineId));

            if (!hasPurchased)
            {
                return BadRequest("Bạn chỉ có thể đánh giá sản phẩm sau khi đã mua và thanh toán thành công.");
            }
